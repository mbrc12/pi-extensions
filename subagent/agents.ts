...
function loadAgentsFromDir(dir: string, source: "user" | "project"): AgentConfig[] {
	...
	for (const entry of entries) {
		...
		// Accept standard YAML lists as well as the existing comma-separated shorthand.
		// Omit `tools` to grant the subagent Pi's normal tool set.
		const toolsValue = frontmatter.tools;
		const tools = Array.isArray(toolsValue)
			? toolsValue.filter((tool): tool is string => typeof tool === "string").map((tool) => tool.trim()).filter(Boolean)
